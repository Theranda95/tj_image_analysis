...
stacks_dir = output_base_dir + "/082024_myh8_maxproj/"; // Path to making the directory
File.makeDirectory(stacks_dir); // Making the output folder

results_dir = output_base_dir + "/082024_myh8_sum/"; 
File.makeDirectory(results_dir); 

masks_dir = output_base_dir + "/082024_myh8_masks/"; 
File.makeDirectory(masks_dir); 

roi_dir = output_base_dir + "/082024_myh8_rois/"; 
...
function flatstack(seriesName) {
	...
	max_name = "MAX_c4_myh8_" + seriesName + ".tif";
    ...
    run("Convert to Mask");
 	...
 	selectWindow(max_name);
 	mask_name = "Mask_c4_myh8_" + seriesName + ".tif";
	...
	run("Close");
	 ...
	 // save ROIs
    roi_save_path = roi_dir + "Mask_c4_myh8_" + seriesName + "_rois.zip";
