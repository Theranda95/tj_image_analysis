...
stacks_dir = output_base_dir + "/072024_achr_maxproj/"; // Path to making the directory
File.makeDirectory(stacks_dir); // Making the output folder

results_dir = output_base_dir + "/072024_achr_results/"; 
File.makeDirectory(results_dir); 

masks_dir = output_base_dir + "/072024_achr_masks/"; 
File.makeDirectory(masks_dir); 

roi_dir = output_base_dir + "/072024_achr_rois/"; 
...
function flatstack(seriesName) {
	...
	run("Close");
	 ...
	 // save ROIs
    roi_save_path = roi_dir + "Mask_c2_achr_" + seriesName + "_rois.zip";
