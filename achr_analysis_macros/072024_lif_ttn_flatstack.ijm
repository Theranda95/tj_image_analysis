...
//This is a version of the previous macro but here I am processing c4 ttn with different parameters 

//This macro makes a max z-project, binary masks and analyzes particles of the mask for a specific channel - in batch
...
stacks_dir = output_base_dir + "/072024_ttn_maxproj/"; // Path to making the directory
...
function flatstack(seriesName) {
	...
	run("Z Project...", "stop=35 projection=[Max Intensity]");
	
	//renaming the max porjection and save as tif
	max_name = "MAX_c4_ttn_" + seriesName + ".tif";
    ...
    //run("Fill Holes");
 	...
 	//saving the mask as a tif file
 	mask_name = "Mask_c4_ttn_" + seriesName + ".tif";
	...
	run("Close");
	 ...
	 // save ROIs
    roi_save_path = roi_dir + "Mask_c4_ttn_" + seriesName + "_rois.zip";
